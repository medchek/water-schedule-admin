<template>
    <div>
        <form
            class="flex flex-col m-auto w-full sm:w-4/5 md:2/3 lg:w-1/2 2xl:w-2/5 rounded p-5 sm:border border-gray-100 bg-transparent sm:bg-white dark:sm:bg-dark-card dark:border-none"
            method="POST"
        ...
        >
            ...
            <div class="space-y-3 mb-6 mt-2">
                <div>
                    ...
                    <p
                        v-if="emailError"
                        class="h-4 min-h-4 text-xs md:text-sm text-red-500 font-semibold mt-0.5 arabic:direction-rtl"
                    >
                        {{ emailError }}
                    </p>
                </div>
            ...
            ></vue-recaptcha>
            <button
                class="h-12 w-full transition-colors bg-blue-500 dark:bg-dark-alt hover:bg-blue-400 dark:hover:bg-blue-light/50 focus:bg-blue-600 dark:focus:bg-dark-header rounded font-semibold text-white text-lg disabled:cursor-not-allowed disabled:bg-bgray-200 disabled:text-bgray-500 disabled:dark:text-bgray-400 disabled:dark:bg-bgray-700"
                type="submit"
        ...
        const login = (recaptchaToken: string) => {
            ...
            AxiosLogin.get("/sanctum/csrf")
                            ...
                            .catch((loginError) => {
                                switch (loginError.response.status) {
                                    case 422:
                                        // invalid credentials
                                        flashSnack({
                                            message: t("general.snack.errors.wrongCredentials"),
                                            type: "error",
                                        });
                                        break;
                                    case 429:
                                    ...
                                    default:
                                        flashSnack({
                                            message: t("general.snack.errors.errorWhileLogin"),
                                            type: "error",
                                        });
                                        break;
                                }
                ...
                .catch((err) => {
                    stopLoading();
                    flashSnack({
                        message: t("general.snack.errors.cors"),
                        type: "error",
                    });
