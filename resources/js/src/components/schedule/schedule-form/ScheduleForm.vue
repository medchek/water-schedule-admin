...
import { fillRemainingPeriod, flashSnack, FormSchedule, isPeriodEmpty, isTimeEmpty, Period, scheduleFormData } from "../../../lib/shared";
import {
  AddSegmentArgs,
  RemoveSegmentArgs,
  ResetAllErrorsArgs,
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const formTitle = computed(() => {
      ...
      const targetSchedule = isNextSelected ? isScheduleLoadedFromStore.next : isScheduleLoadedFromStore.current;
     ...
     */
    const getSchedule = (targetSchedule: TargetSchedule): FormSchedule[] => (targetSchedule === "current" ? currentSchedule : nextSchedule);
    ...
    const handleResetTime = (args: ResetTimeArgs): void => {
      ...
      const schedule = getSchedule(targetSchedule)[dayIndex].schedule[periodIndex][targetTime];
    ...
    const checkBothFormsForErrors = (): { current: boolean; next: boolean } => {
      return { current: doesScheduleHaveErrors(currentSchedule), next: doesScheduleHaveErrors(nextSchedule) };
    };
    ...
    const convertToRequestObject = (schedule: FormSchedule[]): ScheduleDays | null => {
      ...
      const requestObject: ScheduleDays = { sunday: [], monday: [], tuesday: [], wednesday: [], thursday: [], friday: [], saturday: [] };
     ...
     */
    const hasFormBeenUpdated = (schedule: ScheduleDays | null, targetSchedule: TargetSchedule): boolean => {
      const form = serialize(schedule);
      const storeForm = serialize(targetSchedule === "current" ? storeSchedule.value?.current?.schedule : storeSchedule.value?.next?.schedule);
