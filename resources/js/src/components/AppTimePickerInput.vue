    ...
    <div class="relative w-full h-full">
        ...
        <transition name="scale" type="animation" v-if="!disabled">
            <div
                ...
                class="absolute flex flex-col bg-white dark:bg-dark-card w-full h-80 left-0 right-0 mx-auto z-100 mt-2 rounded shadow-xl overflow-hidden dark:shadow-black/40"
                v-if="isOpen"
                ref="pickerRef"
            >
                <p
                    class="picker__HEADER flex items-center justify-center grow-0 bg-blue-500 dark:bg-dark-alt/80 min-h-10 md:min-h-12 text-3xl sm:text-4xl text-white w-full"
                >
                    {{ timePickerDisplay }}
                </p>
                <div
                    class="flex min-h-8 font-semibold bg-bgray-100 dark:bg-dark-cancel text-bgray-600 dark:text-bgray-300 text-sm md:text-base"
                >
                ...
                >
                    <section class="flex flex-col w-full" id="hours-section">
                        <div class="grow h-full max-h-full overflow-y-scroll">
                            <button
                                v-for="n in 24"
                                class="w-full h-6 md:h-8 transition-colors font-semibold text-sm md:text-base hover:bg-bgray-200 focus:bg-bgray-300 dark:hover:bg-bgray-500/50 dark:focus:bg-blue-900/50 text-gray-800 dark:text-bgray-200"
                                :key="n"
        ...
        watch(pickerRef, (newVal) => {
            if (newVal) {
                ...
                const parentCoords = document
                    .getElementById("modal-form-container")
                    ?.getClientRects()[0];
                ...
                const isPickerOverflowBottom =
                    pickerCoords.top + pickerHeight > parentCoords.bottom;
